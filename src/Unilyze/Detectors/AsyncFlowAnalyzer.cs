...
internal sealed record AsyncVoidOccurrence(string MethodName, int Line);
internal sealed record BlockingWaitOccurrence(string MethodName, int Line, string Pattern);
internal sealed record AsyncFlowResult(
    IReadOnlyList<AsyncVoidOccurrence> AsyncVoidMethods,
    IReadOnlyList<BlockingWaitOccurrence> BlockingWaits);

internal static class AsyncFlowAnalyzer
{
